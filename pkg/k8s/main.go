...
func IsValidService(kubeClient kubernetes.Interface, name string) (*api.Service, error) {
...
func IsValidSecret(kubeClient kubernetes.Interface, name string) (*api.Secret, error) {
	ns, name, err := ParseNameNS(name)
	if err != nil {
		return nil, err
	}
...
func GetNodeIP(kubeClient kubernetes.Interface, name string) string {
